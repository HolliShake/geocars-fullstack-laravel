...
Route::middleware(["auth:api", "role:admin,user"])
    ...
    ->group(function () {
        ...
        Route::middleware("role:admin")
            ->put("/Requirement/{id}", "update")
            ->where("id", "[0-9]+");
        Route::middleware("role:admin")
            ->delete("/Requirement/{id}", "destroy")
            ->where("id", "[0-9]+");
    });
...
Route::middleware(["auth:api", "role:admin,user,renter"])
    ...
    ->group(function () {
        ...
        Route::middleware("role:admin,user,renter")
            ->put("/User/{id}", "update")
            ->where("id", "[0-9]+");
        Route::middleware("role:admin")
...
Route::middleware(["auth:api", "role:admin,user,renter"])
    ...
    ->group(function () {
        ...
        Route::middleware("role:user,renter")->post(
            "/UserRequirement",
            "store",
        );
        Route::middleware("role:user,renter")
            ->put("/UserRequirement/{id}", "update")
            ->where("id", "[0-9]+");
        Route::middleware("role:user,renter")
            ->delete("/UserRequirement/{id}", "destroy")
            ->where("id", "[0-9]+");
    });
...
Route::middleware(["auth:api"])
    ...
    ->group(function () {
        ...
        Route::get(
            "/Device/ByCarRental/{car_rental_id}",
            "getByCarRental",
        )->where("car_rental_id", "[0-9]+");
...
Route::middleware(["auth:api"])
    ->controller(DeviceLocationController::class)
    ->group(function () {
...
Route::post("/Stripe/webhook/ingest", [
